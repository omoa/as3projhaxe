...
class BonneProjection extends Projection {
	...
	override public function projectInverse(xyx : Float, xyy : Float, out : Point) : Point {
		if(spherical)  {
			...
			if(Math.abs(Math.abs(out.y) - MapMath.HALFPI) <= EPS10) out.x = 0.
			else out.x = rh * Math.atan2(xyx, xyy) / Math.cos(out.y);
		}
			...
			if((s = Math.abs(out.y)) < MapMath.HALFPI)  {
				s = Math.sin(out.y);
				out.x = rh * Math.atan2(xyx, xyy) * Math.sqrt(1. - es * s * s) / Math.cos(out.y);
			}

			else if(Math.abs(s - MapMath.HALFPI) <= EPS10) out.x = 0.
	...
	override public function initialize() : Void {
		...
		phi1 = MapMath.HALFPI;
		if(Math.abs(phi1) < EPS10) throw new ProjectionError("-23");
		if(!spherical)  {
			en = MapMath.enfn(es);
			m1 = MapMath.mlfn(phi1, am1 = Math.sin(phi1), c = Math.cos(phi1), en);
			am1 = c / (Math.sqrt(1. - es * am1 * am1) * am1);
		}

		else  {
			if(Math.abs(phi1) + EPS10 >= MapMath.HALFPI) cphi1 = 0.
			else cphi1 = 1. / Math.tan(phi1);
		}
