...
class FaheyProjection extends Projection {

	static var TOL : Float = 1e-6;
	...
	override public function projectInverse(xyx : Float, xyy : Float, out : Point) : Point {
		...
		out.x = Math.abs(out.y = 1. - xyy * xyy) < (TOL) ? 0. : xyx / (0.819152 * Math.sqrt(xyy));
		return out;
	}

	function asqrt(v : Float) : Float {
