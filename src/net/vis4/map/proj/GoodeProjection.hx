...
class GoodeProjection extends PseudoCylindricalProjection {

	static var Y_COR : Float = 0.05280;
	static var PHI_LIM : Float = .71093078197902358062;
	var sinu : SinusoidalProjection;
	var moll : MollweideProjection;
	override public function project(lplam : Float, lpphi : Float, out : Point) : Point {
		if(Math.abs(lpphi) <= PHI_LIM) out = sinu.project(lplam, lpphi, out)
		else  {
	...
	override public function projectInverse(xyx : Float, xyy : Float, out : Point) : Point {
		if(Math.abs(xyy) <= PHI_LIM) out = sinu.projectInverse(xyx, xyy, out)
		else  {
